...
- **Permissions**: On Unix the cache directory tree is created `0700` and cache files `0600`, since entries can contain license/copyright text and file paths from private repositories. The cache directory must **not** be placed on a shared or world-writable path; a writable cache root would let another user supply crafted cache files. When a cache directory already exists, Provenant makes a best-effort attempt to tighten it to `0700` on the next run (ignored if the directory is not owned by the current user). A cache tree created by an older version under a permissive umask is therefore hardened automatically; for installs where that best-effort tightening cannot apply, run a one-time `chmod -R go-rwx <cache-dir>`
