...
Direct raw-JSON comparison lane when both scanner outputs already exist:

```bash
cargo run --manifest-path xtask/Cargo.toml --bin compare-outputs -- \
  --scancode-json /path/to/scancode.json \
  --provenant-json /path/to/provenant.json
```
...
- When you already have both raw outputs and only want the reduced diff artifacts, use the direct raw-JSON comparison lane instead of restaging a target and rerunning either scanner.
...
| 0b       | Multi-input `--from-json` merge and top-level recomputation                                                    | 🟢 Verified | combine two or more cached `scancode.json` inputs from the rows above                                                                                                          | High-value because Provenant supports multi-input replay and the compare harness now stages repeated imported JSON inputs in a stable order for both scanners. Keep using representative merged inputs here so top-level recomputation and shaping-time merge behavior stay covered.                                                                                                   |
...
| ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `show-attribution`                                                       | Rust-specific convenience command with no direct ScanCode CLI peer                                                                                                                                                                                                                                                                                                                    |
...
| `provenant serve` lifecycle, health endpoints, and sync local-path scans | Rust-specific long-lived service surface; validate with top-level CLI/integration tests and service-level contract checks rather than ScanCode parity compare lanes                                                                                                                                                                                                                   |
