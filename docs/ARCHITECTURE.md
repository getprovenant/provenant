...
The "backfill" prohibition above is on **parsers**. Reconciling a declared license with a file the manifest **references** is handled later by the post-assembly `apply_package_reference_following` pass (`src/post_processing/reference_following.rs`): it resolves a manifest-declared reference (e.g. a `license-file` / "see LICENSE" pointer) to the referenced file's license. That pass never adopts arbitrary co-located files — an unreferenced sibling `LICENSE`/`README` stays a file-level detection and feeds the clarity score, but is not promoted into a package's declared license (and co-located key-file promotion carries only copyright/holder). One bounded exception, added by [`adr/0010-package-license-from-cohosted-files.md`](adr/0010-package-license-from-cohosted-files.md), is the post-assembly `promote_package_declared_license_from_legal_files` pass: when a package declares and references no license, it adopts a `LICENSE`/`COPYING`/`NOTICE` file from the package's **own directory** (and only when that directory anchors no other package and the legal files agree on one expression), so formats with no declared-license field (Go, autotools, Swift, Bazel) still get a package declared license. See [`adr/0002-extraction-vs-detection.md`](adr/0002-extraction-vs-detection.md).
...
- **TopologyPlan**: Claims directories or multi-directory domains whose package boundaries are defined by project structure instead of plain sibling files (e.g., npm/pnpm workspaces, Cargo workspaces, `go.work`, `pixi.toml`, Hackage project roots, Mix umbrella apps, Dart pub workspaces). For Maven reactors (`pom.xml` `<modules>`), Gradle multiprojects (`settings.gradle(.kts)` `include`), and uv workspaces (`[tool.uv.workspace]` members), it does not claim directories outright — each member still assembles into its own package via the normal per-directory merge — but it attributes source files nested under each member (and under the root) to the nearest enclosing member's package after assembly. Gradle also excludes each module's immediate `build/` output directory from that ownership pass, and honors a literal `project(':path').projectDir = file('...')` remap in the settings script so a relocated subproject resolves at its real directory (dynamic/non-literal remaps are skipped). For uv workspaces, the topology additionally re-attributes the shared root `uv.lock`: a locked entry is owned by every member (or the root, when the root `pyproject.toml` is itself a package) whose own `[project.dependencies]` declare that distribution directly, and an entry no member declares stays hoisted — the same honest shared-lock rule used by the Mix umbrella and Dart workspace topologies.
  - **Poetry monorepos (deferred):** Poetry has no static workspace-member surface analogous to `[tool.uv.workspace].members`; the only cross-project signal is a `path` dependency, which does not by itself declare workspace membership. Provenant does not infer workspace membership from path dependencies, so a Poetry-only monorepo is left as ordinary per-directory packages. For the same reason, `poetry.lock` is excluded from the shared-lock attribution above: without a proven member list there is no honest way to distribute a shared lock across members. A Poetry project that also declares `[tool.uv.workspace]` is handled by the uv workspace path.
