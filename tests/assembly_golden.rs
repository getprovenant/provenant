...
mod tests {
    ...
    #[test]
    fn test_assembly_mix_umbrella() {
        match run_assembly_golden_test("mix-umbrella") {
            Ok(_) => (),
            Err(e) => panic!("Assembly golden test failed for mix-umbrella: {}", e),
        }
    }

    #[test]
    fn test_assembly_mix_umbrella_member_lock() {
        match run_assembly_golden_test("mix-umbrella-member-lock") {
            Ok(_) => (),
            Err(e) => panic!(
                "Assembly golden test failed for mix-umbrella-member-lock: {}",
                e
            ),
        }
    }
