...
| Docs/YAML/JSON formatting       | `npm run format:check`                                                                                                                                          | changed docs/config files                            |
...
| No-default-features smoke       | Same commands as the steps under `Rust Library Tests` in `check.yml`: `cargo check --lib --no-default-features` plus the exact failing no-default-features test | feature-gated parser/scanner code                    |
| Golden Tests (debug)            | `cargo test --test <assembly_golden\|finder_golden\|parsers_golden> --features golden-tests`                                                                    | owning detector/parser/assembly surface              |
| Golden Tests (ci-release)       | `cargo test --test <copyright_golden\|license_detection_golden\|post_processing_golden> --profile ci-release --features golden-tests`                           | owning detector/post-processing surface              |
...
| Golden shard (legacy name)      | Prefer the profile-grouped golden rows above; locally still `cargo test --test <suite> --features golden-tests`                                                 | owning detector/parser/output surface                |
| Docs lint/format                | `npm run check:docs`                                                                                                                                            | markdown/docs formatting                             |
